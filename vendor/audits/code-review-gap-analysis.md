...
1. **Security** — Anderson (*Security Engineering*), Stuttard & Pinto (*WAHH*), Ball (*Hacking APIs*). Baseline checklist distilled per trust-boundary type (web endpoint / API / data layer / config).
2. **Defects** — McConnell (*Code Complete*: defensive programming, error handling), Hunt & Thomas (*Pragmatic Programmer*: assertions, resource balance), Feathers for changes touching untested legacy code (seam quality, characterization tests first).
3. **Performance** — Kleppmann (*DDIA*) for data-touching diffs, Bentley (*Programming Pearls*) for back-of-envelope estimates the diff should survive.
4. **Tests** — *SWE at Google* (testing chapters: test sizes, flakiness, coverage philosophy), Percival & Molina for Python-specific test smells.
