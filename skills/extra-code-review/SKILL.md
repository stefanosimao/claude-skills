...
> **Prose is executable — scope it in.** When the diff touches a skill, agent, prompt, or runbook, the `SKILL.md`/instruction text is **in scope for this axis, not documentation**: it tells a model how to build commands and requests, so an injection hole can live entirely in the prose while every script is clean. **The test is not "is it prose" but "is something else resting on it" — a claim is in scope when it is the stated mechanism a control, credential, or contract depends on, and out of scope when nothing would change if it were wrong.** Wrong code fails where you are looking; wrong load-bearing prose fails on someone else's machine, later, in a way that does not point back. *Re-confirmed 2026-08-04 on `git-guardrails`: the documented coverage of a security control — "the `-C` form doesn't slip past" — was load-bearing precisely because it is what a reader would rely on, and it was false for six of the hook's seven prefixed patterns while the script itself was correct. The documented count was itself wrong on first writing (five) and stayed wrong through two further copies, because each was checked against the sentence above it rather than against the hook.* Check quoting in any documented invocation template (double quotes do not stop `$(...)` or backticks — prefer stdin via a single-quoted heredoc), the named location of any credential or config file, and treat model/tool output that re-enters context as untrusted input. Earned 2026-08-04 on `yt-gemini`, where **all three wrong claims were in prose and every one was load-bearing** — each was the stated mechanism something else rested on: the invocation template was the contract an agent follows literally (shell injection); the docstring's key location was the only thing between a cloner and a committed credential (it named a file this repo never git-excluded); the Surface section's whole premise was process inheritance, and it named `~/.zshrc` while the working config was `~/.zshenv`. The Python was fine in all three. Source-only review passes every one of them.
