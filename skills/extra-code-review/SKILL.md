...
> **Prose is executable — scope it in.** When the diff touches a skill, agent, prompt, or runbook, the `SKILL.md`/instruction text is **in scope for this axis, not documentation**: it tells a model how to build commands and requests, so an injection hole can live entirely in the prose while every script is clean. **Wrong code fails where you are looking; wrong prose fails on someone else's machine, later, in a way that does not point back** — which is why prose defects outlive code defects and why this axis owns them. Check quoting in any documented invocation template (double quotes do not stop `$(...)` or backticks — prefer stdin via a single-quoted heredoc), the named location of any credential or config file, and treat model/tool output that re-enters context as untrusted input. Earned 2026-08-04 on `yt-gemini`, where **all three wrong claims were in prose and every one was load-bearing** — each was the stated mechanism something else rested on: the invocation template was the contract an agent follows literally (shell injection); the docstring's key location was the only thing between a cloner and a committed credential (it named a file this repo never git-excluded); the Surface section's whole premise was process inheritance, and it named `~/.zshrc` while the working config was `~/.zshenv`. The Python was fine in all three. Source-only review passes every one of them.
