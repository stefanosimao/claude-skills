...
|---|---|---|---|---|
| ask-tete | 1.0.2 | 🧪 draft-built | manual | concierge over full catalog (Matt flows + mine); built 2026-07-28. 1.0.1: vendor-availability precondition added to Matt recommendations (FIX-3) · 1.0.2: bare invocation lists the catalog (live read, snapshot fallback); snapshot maintenance wired into skill-forge step 6 |
...
## Build log
- 2026-07-28 (latest): **Skill test plan v1 executed — 12 PASS / 1 FIX BODY, no blockers.** 24 fresh subagents, each given the test prompt verbatim and nothing else; firing measured by grepping each subagent's JSONL transcript for `Skill` tool calls rather than inferred from prose (method banked for v2). **Routing matrix 13/13: 9/9 should-fire, 0/4 false positives.** The two near-miss controls both behaved — "colleague is wrong about Kubernetes" did NOT fire argument-analysis (it offered and asked instead), "birthday message for my mother" did NOT fire negotiation-persuasion. Manual-skill contracts all held: ask-tete handed over the orchestrator command in all three scenarios instead of starting it, extra-code-review pinned the fixed point and paused before spawning, swiss-voting refused cleanly when pushed for a voting recommendation, study fabricated nothing. **Fetch cascade verified at the transcript level:** 2 Drive searches → 1 `read_file_content` on a 33 KB `text/markdown` summary, PDF never fetched, chapter citation traced to the summary's chapter map. **No description rewrites proposed** — with zero misses and zero false positives, every description is measurably correct and Workflow A step 5 was skipped deliberately.
